...
func TestPipenvBuild(t *testing.T) {
	sdk := mocks.NewSDKClient(t)
	sdk.On("ModuleAction", mock.Anything).Return(PythonTestData(string(cidsdk.BuildSystemPipfile), false), nil)
	sdk.On("ExecuteCommand", cidsdk.ExecuteCommandRequest{
...
func TestSetupPyBuild(t *testing.T) {
	sdk := mocks.NewSDKClient(t)
	sdk.On("ModuleAction", mock.Anything).Return(PythonTestData(string(cidsdk.BuildSystemSetupPy), false), nil)
	sdk.On("ExecuteCommand", cidsdk.ExecuteCommandRequest{
		Command: "pip install .",
		WorkDir: "/my-project",
	}).Return(nil, nil)
